...
Wraps the rest of the command pipeline (FastAPI-style): run setup, then `const ctx = await execute({ ctx })`, then teardown. `execute` runs remaining plugin hooks in order, then middleware, validation, and the handler.

**When it runs:** During `executeCommand()`, immediately after parse. Global plugins run first, then command plugins. Each hook **nests** the next: the outer hook’s code after `await execute()` runs after the inner hook and the core pipeline complete.

**What you can do:**

- Access unvalidated `parsedOptions` / `parsedPositionals`
- Access `cli` and the matched `command` definition
- **`return await execute({ ctx })`** to continue. Optional `ctx` merges like middleware `next({ ctx })`.
- **`return halt()`** to stop the command pipeline cleanly with no error (same as command middleware `halt`).
- Use the **resolved value** of `execute()`: post-attempt context — merged validated options over accumulated command `ctx` when those stages completed; otherwise best-effort partial (e.g. validation error before the handler).
- For teardown that must run when the handler throws, wrap `await execute()` in `try` / `finally`.

**Contract:** The hook must invoke `execute` or `halt` (typically by returning its result). Otherwise Cheloni throws `PluginCommandExecutionError`.

**Error handling:** If the hook throws **before** `execute()` finishes successfully, middleware and the handler may not run; Cheloni throws `PluginCommandExecutionError`. If the hook throws **after** `await execute()` resolved, the failure is routed like the old `onAfterCommandExecution` path (`PluginAfterCommandExecutionError` → `cli.onError`) and does not change a successful command outcome. `halt()` is not an error — it is not wrapped as a plugin failure.
...
const authPlugin = definePlugin({
  name: "auth",
  onCommandExecution: ({ command, cli, execute }) => {
...
**Example (inject context + timing after the handler):**
...
```typescript
type PluginCommandExecutionHook = (
  params: PluginCommandExecutionHookParams,
) => Promisable<Record<string, unknown> | void>;
```
...
- `params.execute` - `async (opts?: { ctx?: Record<string, unknown> }) => Promise<Record<string, unknown>>` — continues remaining `onCommandExecution` hooks, then middleware, validation, and handler; merges `ctx` like middleware `next({ ctx })`. Resolves with the post-attempt context snapshot (see `onCommandExecution` above). **Return** this promise (or `halt()`) from the hook.
