...
- Your generated/defined CLI app provides `help`, `version`, and standard cross-cutting flags (like verbosity, config, dry-run) when the std `basicPluginKit` (or equivalent plugins) is used
...
### Extensibility: plugins and plugin kits (framework-level)
...
- Std `basicPluginKit` (the typical default) bundles common developer-experience behavior like help/version/deprecation warnings.
...
- `plugin-kit`: generate a named plugin kit (array of plugins) into source
