...
describe("commitHashSchema", () => {
  it("accepts a valid 40-char lowercase hex hash", () => {
    expect(commitHashSchema.safeParse("da39a3ee5e6b4b0d3255bfef95601890afd80709").success).toBe(
      true,
    );
  });
  ...
  it("rejects non-hex characters", () => {
    expect(commitHashSchema.safeParse("zz39a3ee5e6b4b0d3255bfef95601890afd80709").success).toBe(
      false,
    );
  });
...
describe("commitHashShortSchema", () => {
  ...
  it("accepts a full 40-char hash", () => {
    expect(
      commitHashShortSchema.safeParse("da39a3ee5e6b4b0d3255bfef95601890afd80709").success,
    ).toBe(true);
  });
