...
describe("configPlugin", () => {
  ...
  it("loads config file from cwd", async () => {
    ...
    const cli = await createCli(
      defineCli({
        ...
        command: defineCommand({ name: "__root__", handler }),
      }),
    );
  ...
  it("uses custom configFile name via c12Options", async () => {
    ...
    const cli = await createCli(
      defineCli({
        name: "test-cli",
        plugins: [configPlugin({ c12Options: { configFile: "tasks" } })],
        command: defineCommand({ name: "__root__", handler }),
      }),
    );
  ...
  it("uses defaults when no config file exists", async () => {
    ...
    const cli = await createCli(
      defineCli({
        name: "test-cli",
        plugins: [configPlugin({ c12Options: { defaults: { fallback: "value" } } })],
        command: defineCommand({ name: "__root__", handler }),
      }),
    );
  ...
  it("applies overrides with highest priority", async () => {
    ...
    const cli = await createCli(
      defineCli({
        name: "test-cli",
        plugins: [configPlugin({ c12Options: { overrides: { source: "override" } } })],
        command: defineCommand({ name: "__root__", handler }),
      }),
    );
