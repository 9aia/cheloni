...
describe("createCommand", () => {
  ...
  it("creates nested subcommands", () => {
    ...
    expect([...command.commands.values()].map((c) => c.manifest.name)).toEqual([
      "child1",
      "child2",
    ]);
  });
...
describe("createRootCommand", () => {
  it('creates root command with name "__root__"', () => {
    ...
    expect(command.manifest.name).toBe("__root__");
...
describe("bequeathOptions", () => {
  ...
  it("allows child to override parent bequeathOptions", () => {
    ...
    expect(parentCommand.bequeathOptions.get("verbose")?.definition.schema).toBe(
      parentVerbose.schema,
    );
    expect(childCommand.bequeathOptions.get("verbose")?.definition.schema).toBe(
      childVerbose.schema,
    );
  });
