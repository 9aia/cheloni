...
describe("executeCli", () => {
  ...
  it("shows error when no root command", async () => {
    ...
    await expect(executeCli({ cli, args: ["--flag"] })).rejects.toThrow("process.exit called");
  ...
  it("shows deprecation warning for deprecated positional when provided", async () => {
    ...
    const warnMessages = consoleWarnSpy.mock.calls.map((call: unknown[]) => String(call[0]));
    expect(
      warnMessages.some((m: string) => m.includes("Deprecated:") && m.includes("Use new argument")),
    ).toBe(true);
    expect(handler).toHaveBeenCalledOnce();
  });
  ...
  it("continues error-handler plugins if an onError hook throws (and reports hook error to cli.onError)", async () => {
    ...
    const hookErrorCall = cliOnError.mock.calls.find((call: unknown[]) => {
      const first = call[0] as { error?: unknown } | undefined;
      return first?.error instanceof PluginHookError;
    });
