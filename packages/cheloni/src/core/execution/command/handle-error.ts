...
function handleSchemaError({ error, command }: HandleValidationErrorOptions) {
  ...
  for (const err of error.issues) {
    ...
    const pathSegments =
      err.path?.map((p: unknown) => {
        if (typeof p === "object" && p !== null && "key" in p) {
          return (p as { key: PropertyKey }).key;
        }
        return p as PropertyKey;
      }) || [];
    ...
    if (pathSegments.length === 0) {
      ...
      const optionsManifest = command.definition.options
        ? getOptionsManifest(command.definition.options)
        : [];
