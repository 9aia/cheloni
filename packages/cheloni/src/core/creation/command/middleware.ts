 ...
 * ```
 */
export type Middleware<TCtxOut extends UnknownRecord = UnknownRecord> = (
...
export type InferMiddlewareContext<T> = T extends (
  ...args: any[]
) => Promisable<MiddlewareResult<infer R>>
  ? R
  : UnknownRecord;

/** Recursively intersects context types from a tuple of middleware. */
export type InferComposedContext<T extends AnyMiddleware[]> = T extends [
